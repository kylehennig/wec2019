...
class GuessBoard:
    ...
    def __init__(self, board, basin_count):
        self.board = board
        ...
        for i in range(self.basin_count):
            ...
            for j in range(self.basin_count):
                self.guess_board[i].append(GuessNode(i, j, board))
        ...
        for i in range(self.basin_count):
            for j in range(self.basin_count):
                ...
                if i != 0 and j != self.basin_count - 1 and \
                        self.guess_board[i - 1][j + 1].basin:
                    count += 1
    ...
    def check_for_basin(self):
        ...
        for i in range(self.basin_count):
            for j in range(self.basin_count):
                if self.guess_board[i][j].adjacent_basins == \
                        self.guess_board[i][j].adjacent_unvisited:
